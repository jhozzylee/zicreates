export const portfolioItems = [
    {
      id: 1,
      type: "Branding",
      brand: "K.I Computing",
      category: "Brand Identity",
      caption: "Brand identity and visual design.",
      description:
        "K.I. Computing is a Wyoming-based IT firm specializing in cybersecurity, compliance, and managed IT solutions for small businesses. Zi Creates partnered with the brand to modernize its visual identity and strengthen its online presence.",
      summaryHighlights: [
        {
          title: "Packaging design",
          text: "Visual direction, content design, and consistent brand presence across Social Media",
        },
        {
          title: "Visual identity",
          text: "Logo, typography, and brand colors",
        },
      ],
      involvement: ["Brand Strategy", "Social Media Rebranding ", "Visual Identity"],
      src: "/assets/ki-computing2.png",
      thumbnail: "/assets/ki-computing.png",
      related: [2, 4, 8],
    },
    {
      id: 2,
      type: "Websites",
      brand: "TechNest",
      category: "Website Redesign",
      caption: "Website redesign for tech startup.",
      description:
        "TechNest wanted a sleek, performance-driven redesign that would appeal to developers and investors alike. We delivered a React-powered site with a strong UX/UI foundation.",
      summaryHighlights: [
        {
          title: "Stack",
          text: "Built with React, TailwindCSS, and Framer Motion",
        },
        {
          title: "UX Research",
          text: "Targeted interviews and user testing",
        },
      ],
      involvement: ["Web Design", "Frontend Development", "UI/UX Strategy"],
      videoUrl: "https://player.vimeo.com/video/987654321",
      thumbnail: "/assets/project2.jpg",
      related: [1, 9],
    },
    {
      id: 3,
      type: "Motion Designs",
      brand: "Webshapers",
      category: "Promo Video",
      caption: "Promo motion graphic video.",
      description:
        "Webshapers is a forward-thinking digital agency focused on helping brands unlock online growth through performance-driven strategies and cutting-edge web experiences. Zi Creates collaborated with Webshapers to enhance its brand visuals and create dynamic content that better communicates its bold, energetic identity.",
      summaryHighlights: [
        {
          title: "Editing style",
          text: "kinetic motion, tech-style transitions, bold typography with upbeat music",
        },
        {
          title: "Animation",
          text: "UI mockups animated in After Effects",
        },
      ],
      involvement: ["Storyboarding", "Motion Design", "Sound Design"],
      videoUrl: "https://player.vimeo.com/video/1102249418?autoplay=1",
      src: "/assets/Ws-03.png",
      related: [5, 6],
    },
    {
      id: 4,
      type: "Product Designs",
      brand: "TechNest",
      category: "Product Landing Page",
      caption: "Product design for SaaS product.",
      description:
        "We created a conversion-focused product page for TechNest’s new analytics tool. Designed to inform and convert.",
      summaryHighlights: [
        {
          title: "Conversion focused",
          text: "Clear CTA placement and visual hierarchy",
        },
      ],
      involvement: ["Product UI", "Landing Page", "UX Design"],
      videoUrl: "https://player.vimeo.com/video/998877665",
      thumbnail: "/assets/project2.jpg",
      related: [2, 9],
    },
    {
      id: 5,
      type: "Designs",
      brand: "Nova App",
      category: "Promo Snippets",
      caption: "App promo motion graphic video.",
      description:
        "Short cutdowns of the full Nova promo video tailored for TikTok and Instagram Reels.",
      summaryHighlights: [
        {
          title: "Format",
          text: "Vertical 9:16 ratio for mobile-first experience",
        },
      ],
      involvement: ["Video Editing", "Reformatting", "Platform Optimization"],
      videoUrl: "https://player.vimeo.com/video/112233445",
      thumbnail: "/assets/motion1-thumb.jpg",
      related: [3],
    },
    {
      id: 6,
      type: "Motion Designs",
      brand: "Nova App",
      category: "Explainer Animation",
      caption: "App promo motion graphic video.",
      description:
        "An animated walkthrough of the onboarding experience and value proposition of Nova App.",
      summaryHighlights: [
        {
          title: "Voiceover sync",
          text: "Narration synced to UI interactions",
        },
      ],
      involvement: ["Scriptwriting", "Voiceover", "Motion Design"],
      videoUrl: "https://player.vimeo.com/video/112233445",
      thumbnail: "/assets/motion1-thumb.jpg",
      related: [3, 5],
    },
    {
      id: 7,
      type: "Designs",
      brand: "TechNest",
      category: "Design Mockups",
      caption: "Mockups for startup launch deck.",
      description:
        "A set of visual assets used in TechNest’s investor pitch deck including mockups and infographics.",
      summaryHighlights: [
        {
          title: "Pitch Ready",
          text: "Investor-focused UI storytelling",
        },
      ],
      involvement: ["Graphic Design", "Mockup Creation", "Slide Design"],
      videoUrl: "https://player.vimeo.com/video/556677889",
      thumbnail: "/assets/project2.jpg",
      related: [2, 4],
    },
    {
      id: 8,
      type: "Branding",
      brand: "Crestville Appartments",
      category: "Visual Identity",
      caption: "Brand identity and Visuals design.",
      description:
        "Crestville Apartments, a short‑let real estate brand under Assethaus in Lagos, specializes in affordable luxury serviced apartments in key locations (Surulere & Ilupeju). Since its inception, it has rapidly scaled with high occupancy rates and strong repeat bookings.",
      summaryHighlights: [
        {
          title: "Consistency",
          text: "Defined positioning of affordability with luxury, targeting middle‑high earners",
        },
      ],
      involvement: ["Visual Identity", "Sub-branding", "Collateral Design"],
      src: "/assets/Crestville2.png",
      thumbnail: "/assets/Crestville1.png",
      related: [1],
    },
    {
      id: 9,
      type: "Websites",
      brand: "TechNest",
      category: "Web Development",
      caption: "Website redesign for tech startup.",
      description:
        "Final deployment of the TechNest website, optimized for performance and mobile experience.",
      summaryHighlights: [
        {
          title: "Lighthouse Score",
          text: "95+ on mobile and desktop",
        },
      ],
      involvement: ["Frontend Dev", "Accessibility", "Performance Tuning"],
      videoUrl: "https://player.vimeo.com/video/987654321",
      thumbnail: "/assets/project2.jpg",
      related: [2, 4],
    },
    {
      id: 10,
      type: "Motion Designs",
      brand: "Nova App",
      category: "Microinteractions Reel",
      caption: "App promo motion graphic video.",
      description:
        "We stitched together various microinteractions used in Nova App for a UI showcase video.",
      summaryHighlights: [
        {
          title: "Micro UX",
          text: "Tiny but delightful interactions",
        },
      ],
      involvement: ["UI Recording", "Editing", "Motion Design"],
      videoUrl: "https://player.vimeo.com/video/112233445",
      thumbnail: "/assets/motion1-thumb.jpg",
      related: [3, 6],
    },
  ];
  